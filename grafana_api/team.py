...
class Team:
    ...
    def add_team(self, team: TeamObject) -> int:
        ...
        if team is not None:
            api_call: dict = Api(self.grafana_api_model).call_the_api(
                ...
                json.dumps({"name": team.name, "email": team.name}),
            )
    ...
    def get_team_members(self, id: int) -> list:
        ...
        if id != 0:
            ...
            if api_call == [] or api_call[0].get("userId") is None:
                logging.error(f"Check the error: {api_call}.")
                raise Exception
            else:
                return api_call
    ...
    def update_team_preferences(
        ...
        if id != 0 and (
            ...
            if theme is not None:
                team_preferences.update({"theme": theme})
            ...
            if timezone is not None:
                team_preferences.update({"timezone": timezone})
    ...
    def get_external_groups(
        ...
        if id != 0:
            ...
            if isinstance(api_call, list) is False or api_call == []:
                logging.error(f"Check the error: {api_call}.")
                raise Exception
            else:
                return api_call
    ...
    def add_external_group(
        ...
        if id != 0 and team_group is not None and len(team_group) != 0:
            api_call: dict = Api(self.grafana_api_model).call_the_api(
                f"{APIEndpoints.TEAMS.value}/{id}/groups",
                RequestsMethods.POST,
                json.dumps({"groupId": team_group}),
                response_status_code=True,
            )

            status_code: int = api_call.get("status")

            external_group_status_dict: dict = {
