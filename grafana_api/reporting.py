...
class Reporting:
    ...
    def send_report(
        ...
        if id != 0 and (
            ...
            if emails is not None and len(emails) != 0:
                result: dict = {"id": id, "emails": emails}
            else:
                result: dict = {"id": id, "useEmailsFromReport": use_emails_from_report}
    ...
    def get_report_branding_settings(self) -> dict:
        ...
        if api_call != {} and api_call.get("id") is not None:
            return api_call
        elif 401 <= status_code <= 500:
            logging.error(reporting_status_dict.get(status_code))
            raise Exception
    ...
    def save_report_branding_settings(self, branding_settings: ReportBrandingSettings):
        ...
        if (
            ...
            report_branding_settings_object: dict = {
                    "branding": {
        ...
        if (
            ...
            if report.frequency == "custom":
                ...
                if report.workdays_only is not None:
                    report_object.update(
                        {"schedule": {"workdaysOnly": report.workdays_only}}
                    )
                else:
            ...
            for dashboard in report.dashboards:
                dashboard_object: dict = {}

                if len(dashboard.dashboard_uid) != 0:
                    dashboard_object.update(
                        {"dashboard": {"uid": dashboard.dashboard_uid}}
                    )
                else:
            ...
            report_object.update({"dashboards": dashboard_list})
