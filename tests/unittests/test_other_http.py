...
class OtherHTTPTestCase(TestCase):
    ...
    def test_get_frontend_settings(self, call_the_api_mock):
        ...
        self.assertEqual(dict({"allowOrgCreate": True}), other_http.get_frontend_settings())
    ...
    def test_renew_login_session_based_on_remember_cookie(self, call_the_api_mock):
        ...
        mock: Mock = Mock()
        mock.json = Mock(return_value=dict({"message": "Logged in"}))

        call_the_api_mock.return_value = mock

        self.assertEqual(None, other_http.renew_login_session_based_on_remember_cookie())

    @patch("src.grafana_api.api.Api.call_the_api")
    def test_renew_login_session_based_on_remember_cookie_no_valid_result(self, call_the_api_mock):
        model: APIModel = APIModel(host=MagicMock(), token=MagicMock())
        other_http: OtherHTTP = OtherHTTP(grafana_api_model=model)
