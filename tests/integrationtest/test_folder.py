...
class FolderTest(TestCase):
    ...
    def test_get_folders(self):
        self.assertEqual([{"id": 72, "uid": "6U_QdWJnz", "title": "Github Integrationtest"}], self.folder.get_folders())

    def test_get_folder_by_uids(self):
        self.assertEqual({"canAdmin": True,
                          "canDelete": True,
                          "canEdit": True,
                          "canSave": True,
    ...
    def test_b_update_folder(self):
        self.folder.update_folder("test2", self.folder.get_folders()[1].get("uid"), version=1)
    ...
    def test_get_folder_permissions(self):
        self.assertEqual([{"created": "2022-06-14T21:32:34+02:00",
                           "folderId": 72,
                           "inherited": False,
                           "isFolder": True,
                           "permission": 1,
                           ...
                           "userEmail": "",
                           "userId": 0,
                           "userLogin": ""}], self.folder.get_folder_permissions("6U_QdWJnz"))

    def test_d_update_folder_permissions(self):
        permission_dict: dict = dict({
            "items": [
                {
                    "role": "Viewer",
                    "permission": 2
        ...
        self.assertEqual(2, self.folder.get_folder_permissions("6U_QdWJnz")[0].get("permission"))
        ...
        self.assertEqual(1, self.folder.get_folder_permissions("6U_QdWJnz")[0].get("permission"))

    def test_get_folder_id_by_dashboard_path(self):
        self.assertEqual(72, self.folder.get_folder_id_by_dashboard_path("Github Integrationtest"))

    def test_get_all_folder_ids_and_names(self):
        self.assertEqual(list([{'id': 72, 'title': 'Github Integrationtest'}]),
